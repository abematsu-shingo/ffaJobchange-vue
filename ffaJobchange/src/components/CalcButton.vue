...
const calcStatus = () => {
  // カプセルなしで、目標値×50000
  props.uncpStatuses.forEach((item, index) => {
    // Null合体演算子(??)：目標値がnullや空文字の場合0を返す。
    item.value = (props.targetStatuses[index].value ?? 0) * 50000
  })

  // カプセルなしの合計値
  uncpSum.value = props.uncpStatuses.reduce((sum, item) => {
  ...
  // カプセル込みで、目標値×50000
  props.cpStatuses.forEach((item, index) => {
    const currentIndex = props.currentStatuses[index].value ?? 0 // 現在値の各ステータス
    const targetIndex = props.targetStatuses[index].value ?? 0 // 目標値の各ステータス
    ...
    } else if (targetIndex < formula || props.targetStatuses[index].value === 0) {
  ...
  // カプセル込みの合計値
  cpSum.value = props.cpStatuses.reduce((sum, item) => {
    return sum + (item.value ?? 0)
  }, 0)
